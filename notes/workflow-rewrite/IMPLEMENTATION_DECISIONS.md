...
|---|---|---|---|---|---|---|---|
| Resolved | Parser, aggregate, diagnostic, and generated-schema limits | W01 | Each YAML document is limited to 524,288 UTF-8 bytes, AST depth 64, and 20,000 AST nodes. Diagnostic results contain at most 100 entries including the truncation marker, 16 related entries, and 16 dependency-chain entries; every diagnostic string is bounded to 2,048 UTF-8 bytes without splitting code points. Public offsets are zero-based UTF-16 half-open ranges; lines/columns are one-based. Editor schemas are generated under `schemas/` from `src/config/schema.ts` by `scripts/generate-config-schemas.mjs`. | Focused parser/schema tests cover byte/depth/node/diagnostic floods, complete mapping-key traversal, exact ranges, runtime/editor parity through TypeBox and Ajv Draft 2020-12, and schema drift. `just verify` passes on Node 24; the config lane passes on Node 20.19. Cross-platform review gzip output uses `fflate` and package verification passes on Node 20/22/24. | `tests/config-diagnostics.test.ts`; `tests/config-yaml.test.ts`; `tests/config-schema.test.ts`; `tests/config-schema-generated.test.ts`; `Justfile` `config-schema-verify` and `test-node-compat` | Schema v1 | 2026-07-17 |
