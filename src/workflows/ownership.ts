...
function defaultDead(owner: RuntimeOwner): boolean { return processIdentityIsDead(owner); }
export function acquireRuntimeOwnership(root: string, sessionId: string, options: AcquireOwnershipOptions = {}): OwnershipResult {
  ...
  return withCrossProcessFileLock(path, () => {
    ...
    const pid = options.pid ?? process.pid; const timestamp = new Date(now).toISOString(); const owner = Object.freeze({ formatVersion: 1 as const, sessionId, pid, processMarker: options.processMarker ?? currentProcessMarker(pid), bootNonce: options.bootNonce ?? currentBootNonce(), ownerNonce: options.nonce ?? randomUUID(), generation: randomUUID(), acquiredAt: timestamp, heartbeatAt: timestamp }); writeAtomic(path, owner); return Object.freeze({ ok: true, reason: existing ? "verified dead stale owner recovered" : "runtime ownership acquired", owner, recovered: Boolean(existing), ...(existing ? { previousOwner: existing } : {}) });
