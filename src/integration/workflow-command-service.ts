...
import { probeDashboard } from "../engine/dashboard";
...
export function createPiWorkflowRuntimeCommandAuthority(): WorkflowRuntimeCommandAuthority {
  return Object.freeze({
    ...
    async dashboardAvailable(): Promise<boolean> { return Boolean(await probeDashboard()); },
...
export function createLinkedWorkflowCommandServices(pi: ExtensionAPI, projectRoot: string, projectId: string, authority: WorkflowRuntimeCommandAuthority = createPiWorkflowRuntimeCommandAuthority()): WorkflowCommandServices {
  ...
  const owner = () => ({ pid: process.pid, processMarker: `pi-hive-${process.pid}`, nonce: ownerNonce, verifyDead: () => false });
  ...
  return {
    ...
    async exit(context) { const ctx = requireContext(context); await exitWorkflowSession({ projectRoot, currentPiSessionId: currentPiSessionId(ctx), ownerNonce, adapter: createPiSessionNavigationAdapter(ctx) }); return "Returned to the linked normal chat session"; },
