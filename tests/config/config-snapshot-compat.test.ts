...
const runtime = { model: { defaultModel: "provider/model", defaultThinking: "off", find: (id: string) => id === "provider/model" ? { id, contextWindow: 300000, thinking: ["off"] } : undefined, canActivate: () => true, estimateTokens: () => 0 }, knowledgeAvailable: () => true, workspaceAvailable: () => true, artifactProfileAvailable: () => true };
