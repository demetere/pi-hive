...
test("Bun server coverage gate rejects low and missing runtime coverage", () => {
  const low = { ...THRESHOLDS, "src/observability/server/workflow-runtime.ts": 89 };
  const failed = run(low);
  assert.equal(failed.status, 1);
  assert.match(failed.stderr, /workflow-runtime\.ts: 89\.00% lines/);

  const { "src/observability/server/workflow-runtime.ts": _missing, ...withoutRuntime } = THRESHOLDS;
  const missing = run(withoutRuntime);
  assert.equal(missing.status, 1);
  assert.match(missing.stderr, /workflow-runtime\.ts: missing/);
});
